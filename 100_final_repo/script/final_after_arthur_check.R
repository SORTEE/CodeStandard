...
test_that("Treatment_relevelled reflects Treatment", {
  expect_true(all(cat_data_surv$Treatment_relevelled == cat_data_surv$Treatment))
  })
...
test_that("Treatment_relevelled reflects Treatment", {
  expect_true(all(cat_data_pupa$Treatment_relevelled == cat_data_pupa$Treatment))})
test_that("MismTreat_squared equals MismTreat_noNeg squared", {
...
if(save_figures) {
  ggsave(
    filename="output/fig/PupWeight_wpred_rev.png",
...
# Arthur's note: I would remove session info
