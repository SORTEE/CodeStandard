...
anova1 <- drop1(glm1,test = "Chi") |>
  as.data.frame # interaction not significant; the use of Chi-square test to determine statistical significance should be explicitly mention in the paper
anova1$mod <- "glm1"

glm2 <- update(glm1, ~ . -MismTreat1:PhotoTreat - MismTreat2:PhotoTreat) # simplify model
anova2 <- drop1(glm2, test="Chi") |> 
  as.data.frame #no effect of PhotoTreatment, but effect of MismTreat and MismTreat^2
anova2$mod <- "glm2"
...
predicted_survival$samplesize <- aggregate(CaterpillarID~MismTreat, 
...
raw_weight_figure <- ggplot(data = pupa_weight,
  ...
  geom_jitter(data = pupa_data, aes(col = PhotoTreat), 
              alpha = 0.3, size = 3, height = 0, width = 0.25)+ #alpha=0.3, size=2, height=0, width=0.25
...
anova1 <- anova(lm1) |> 
  as.data.frame() # interaction not significant
anova1$mod <- "lm1"

lm2 <- update(lm1, ~ . - MismTreat1:PhotoTreat - MismTreat2:PhotoTreat) # simplify model
anova2 <- anova(lm2) |>
  as.data.frame() # Squared mismatch not significant
anova2$mod <- "lm2"

lm3 <- update(lm2, ~ . - MismTreat2) # simplify model
anova3 <- anova(lm3) |> 
  as.data.frame() # PhotoTreat and MismTreat significant
anova3$mod <- "lm3"

# Still there if exclude first time point with low sample size?
lm4 <- lmer(PupaWeight ~ -1 + MismTreat1 + PhotoTreat + (1|MotherID),
...
pupa_weight_plot <- raw_weight_plot + #geom_line(data=pred1, aes(y=pred)) +
  geom_smooth(data = pred1, aes(y = pred, col = PhotoTreat), se = FALSE, method = lm) +
...
#-----------------------------------
relative_fit <- merge(glm.fit[, c("MotherID", "MismTreat", "survpred")], 
...
relative_fit_plot <- ggplot(data = relative_fit, 
                            aes(x = MismTreat, y = rel)) +
  ...
  geom_errorbar(data = RelFit_means, 
                aes(ymax = rel + se, ymin = rel - se), width = 0.3) +
  ...
  geom_text(data = RelFit_means, aes(label = samplesize, y = rel + 0.12), 
            colour = "black", size = 5, fontface = "bold") + # number of caterpillars
  geom_hline(yintercept = 1, linetype = "dashed")+
  labs(y = "Relative fitness", x = "Mismatch with oak budburst date (days)") +
  scale_y_continuous(lim = c(0, 1.21), breaks = seq(0, 1.6, by = 0.2)) +
  scale_x_continuous(breaks = seq(-4, 5, by = 1)) + #lim=c(-0.1,1.3)
